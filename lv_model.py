...
def run_LV():
    ...
    for i in range(1000):
        print(i)
        # Sample params 
        sampled_params = sample_LV_input_params()

        # Unpack initial conditions
        init_H_1 = sampled_params['H_1']
        init_H_2 = sampled_params['H_2']
        init_P_1 = sampled_params['P_1']
        y0 = [init_H_1, init_H_2, init_P_1]

        # Simulate
        t = np.arange(0, 100, 0.001)
        y0 = [0.5, 1.0, 1.0]
        sol = odeint(one_pred_two_prey, y0, t, (sampled_params,))
        ...
        p_y0 = y0[:]
        p_y0[idx] = p_y0[idx] + pert_magnitude * coin_flip

        p_sol = odeint(one_pred_two_prey, p_y0, t, (sampled_params,))
        ...
        end_state_diff = abs(sol[-1][idx] - p_sol[-1][idx])
        initial_perturbation = abs(pert_magnitude * coin_flip)

        if initial_perturbation < end_state_diff and end_state_diff > 0.1:
            print(i)
            print("Chosen idx: ", idx, " perturbation: ", pert_magnitude * coin_flip)
            print("diff idx: ", sol[-1][idx] - p_sol[-1][idx])
            print("")
            ...
            axes[1].plot(t, p_sol[:, 0])
            axes[1].plot(t, p_sol[:, 1])
            axes[1].plot(t, p_sol[:, 2])
            fig.tight_layout()
            plt.savefig(str(i) + ".pdf", dpi=500)
            fig.clf()
            plt.close()
...
def run_LV_separation(idx=0):
    ...
    output = calculate_separation(one_pred_two_prey, y0, sampled_params, min_species_value=1e-7, t_init=t_init, dt=dt, sep_len=sep_len)
    separation_coefficients = output[1]
    print(np.mean(separation_coefficients))
...
def run_lorenz_map(idx):
    ...
    output = calculate_separation(lorenz_map, y0, sampled_params, 
        t_init=t_init, dt=dt, sep_len=1000)
